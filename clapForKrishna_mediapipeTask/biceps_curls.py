...
# with open('robofest-wildcard-cdfea2abe918.json') as source:
#     info = json.load(source)
# credentials = service_account.Credentials.from_service_account_info(info)

# client = pygsheets.authorize(service_account_file='service_account.json')

# spreadsheet_url = "https://docs.google.com/spreadsheets/d/10zcFImr_eulTsLkdimI0J-p3KWfX5TA4z7gDk-wYLDo/edit#gid=0"
# sheet_data = client.sheet.get('10zcFImr_eulTsLkdimI0J-p3KWfX5TA4z7gDk-wYLDo')

# sheet = client.open_by_url('https://docs.google.com/spreadsheets/d/10zcFImr_eulTsLkdimI0J-p3KWfX5TA4z7gDk-wYLDo/edit#gid=0')
# print(sheet)
...
'''
x = [[(countSets-1),int(countReps) + targetReps*(countSets-1), current_time , facerecog.id]]
response = sheet.values().update(spreadsheetId=SAMPLE_SPREADSHEET_ID,range="Sheet1!A2:E2",valueInputOption="USER_ENTERED",body={"values":x}).execute()
'''

x = [[(countSets-1),int(countReps) + targetReps*(countSets-1), current_time ]]
response = sheet.values().update(spreadsheetId=SAMPLE_SPREADSHEET_ID,range="Sheet1!B2:D2",valueInputOption="USER_ENTERED",body={"values":x}).execute()
